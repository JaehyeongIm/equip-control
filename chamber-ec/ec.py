#!/usr/bin/env python3
"""
Equipment Controller (EC) — 히터 과온 알람 진단 및 안전 복구 시스템
Usage: python3 ec.py <serial_port>
  예)  python3 ec.py /dev/tty.usbmodem*

DATA protocol (12 fields):
  DATA:temperature,setpoint,state,alarm,duty,elapsed,reached,reach_s,
       peak_temp,overshoot,settled,settle_s
"""
...
_state = {
    ...
    "elapsed":   None,        # seconds after START
    "reached":   None,        # 0 or 1
    "reach_s":   None,        # target reach time (s)
    "peak_temp": None,        # peak temperature after target reached
    "overshoot": None,        # peak_temp - setpoint
    "settled":   None,        # 0 = not yet, 1 = settled, None = no run
    "settle_s":  None,        # settling time from START (s)
    "fw_state":  "UNKNOWN",
    "alarm":     "NONE",
    "events":    [],
    "sensor_logs": [],
...
ALM_INFO = {
    "ALM-01": ("TEMP_WARNING", "온도 > SP+1°C, 5초 지속 → 냉각팬 ON",  "온도 ≤ SP+1°C 복귀 시 팬 OFF, 자동 해제"),
    "ALM-02": ("TEMP_ALARM",   "온도 > SP+2°C, 10초 지속",              "온도 ≤ SP-2°C 복귀 후 RESET"),
...
def _write_sensor_csv(ts, temp, sp, duty, fw_state, alarm,
                      elapsed, reached, reach_s, peak_temp, overshoot,
    ...
    _csv_writer.writerow([
        ...
        "" if settle_s is None else f"{settle_s:.1f}",
        raw_line,
    ])
    _csv_file.flush()
...
def _normalize_duty(raw):
    """100보다 큰 값은 0~1000 compare 값으로 보고 10으로 나눠 percent로 표시."""
    if raw is None:
        return None
    duty = float(raw)
    if duty > 100.0:
...
def _parse(line: str):
    ...
    with _lock:
        if line.startswith("DATA:"):
            parts = line[5:].split(",")
            # Fields: temp,sp,state,alarm[,duty[,elapsed[,reached[,reach_s
            #         [,peak_temp[,overshoot[,settled[,settle_s]]]]]]]]
            if len(parts) >= 4:
                try:
                    ...
                    if len(parts) >= 11 and parts[10].strip() != "":
                        settled = int(float(parts[10].strip()))

                    settle_s = None
                    if len(parts) >= 12:
                        settle_s = _parse_float(parts[11].strip())

                    # peak/overshoot EC 보조 계산 (FW가 안 보내는 경우)
                    ...
                    if temp is not None and sp is not None and duty is not None:
                        ...
                        settle_txt    = f" Settle={settle_s:.1f}s" if settled and settle_s is not None else ""
                        sensor_msg = (
                            f"[{ts}] Temperature={temp:.1f}C Setpoint={sp:.1f}C "
                            f"Duty={duty:.1f}% State={fw_state} Alarm={alarm}"
                            f"{elapsed_txt}{reach_txt}{overshoot_txt}{settle_txt}"
                        )
                    else:
            ...
            if line.startswith("ACK:START"):
                ...
                _state["settled"]   = None
                _state["settle_s"]  = None
            elif line.startswith("EVENT:SETTLED"):
                parts = line.split(",")
                if len(parts) >= 2:
                    try:
                        _state["settled"]  = 1
                        _state["settle_s"] = float(parts[1])
                    except ValueError:
                        pass
...
def _ui(stdscr, ser):
    ...
    while True:
        ...
        with _lock:
            ...
            settled     = _state["settled"]
            settle_s    = _state["settle_s"]
        ...
        settle_str    = f"{settle_s:.1f}s" if settle_s is not None and settled else "---"

        _safe_addstr(stdscr, row, 2, "[State]       FW: ")
        _safe_addstr(stdscr, row, 20, fw_state, sc | curses.A_BOLD)
        row += 1

        _safe_addstr(stdscr, row, 2,
                     f"[Sensor]      Temperature: {temp_str}   Setpoint: {sp_str}   Duty: {duty_str}",
                     sc | curses.A_BOLD)
        row += 1

        _safe_addstr(stdscr, row, 2,
                     f"[Run]         Elapsed: {elapsed_str}   Reach: {reach_str}   Settle: {settle_str}")
        row += 1

        _safe_addstr(stdscr, row, 2,
                     f"[KPI]         Peak: {peak_str}   Overshoot: {overshoot_str}")
        row += 1
            ...
            if fw_state == "ALARM" and alarm in ("ALM-02", "ALM-03"):
                ...
                if temp is not None and reset_thr is not None:
                    ...
                    _safe_addstr(stdscr, row, 4,
                                 f"[복구 조건] Temperature ≤ {reset_thr:.1f}°C : {mark}"

...
### 포함 범위
- STM32 펌웨어: 센서 수집, 액추에이터 제어, EC와의 내부 프로토콜 통신
...
5. 정량 수용 기준을 통해 검증 가능한 시스템을 설계한다.
...
### FR-011 명령 수신 및 실행
EC로부터 수신한 액추에이터 제어 명령을 실행하고 응답을 반환해야 한다.
...
### FR-018 Equipment State 초기화
EC 시작 시 Equipment State를 `INIT`으로 설정해야 한다.

### FR-019 초기화 완료 후 IDLE 전이
...
## 6.6 [EC] Control State 관리

### FR-025 Control State 제공
...
### FR-029 ALARM/ERROR 중 Control State 전환 제한
...
### AC-002 HSMS 연결
Host 실행 후 S1F13/F14 Online 절차가 완료되어야 한다.
...
## 14.1 요구사항 ID 체계
- FR: Functional Requirement (FR-001 ~ FR-055)
- NFR: Non-Functional Requirement (NFR-001 ~ NFR-012)
- AC: Acceptance Criteria (AC-001 ~ AC-012)
...
### 2단계: EC 소프트웨어 MVP
- 상태 머신 (FR-018~024)
- 알람/이벤트 관리 (FR-030~039)
- HSMS 서버 + SECS-II 기본 S/F (FR-040~045)

### 3단계: Host 프로그램
- HSMS 클라이언트 + 상태/센서/알람/이벤트 표시 (FR-051~055)

### 4단계: 통합 검증
- 수용 기준(AC-001~012) 전항목 정량 검증
